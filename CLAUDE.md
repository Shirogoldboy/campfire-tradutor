...
- [x] **Rodada v0.4 (2026-08-10)** — EULA Seção 10 e Termos Seção 9 ajustados pra conformidade com o CDC (art. 51, Lei nº 8.078/1990): distingue imprecisão inerente de tradução por IA (não é defeito) de falhas técnicas reais do software; Privacidade Seção 3 ganhou base legal específica da LGPD (art. 33, VIII c/c art. 7º, V) no lugar do art. 33 genérico, e Seção 5 ganhou texto sobre exclusão de dados de menor tratado sem consentimento — a frase sobre "aviso de restrição de idade no primeiro uso" foi removida após busca no código confirmar que essa tela não existe em nenhum dos dois apps (não dá pra declarar no documento algo que o software não faz). `Políticas/politica_do_dicionario.md` Seção 2: nome "Campfire Smart Dictionary" confirmado por Caio, colchete de pendência removido — **sem mais pendências de conteúdo pra v1.0**. Aviso de Direitos Autorais alinhado de v0.2 pra **v0.3** (revisão jurídica já concluída anteriormente, sem alteração de conteúdo nesta rodada). Termos/EULA/Privacidade → **v0.4**; Dicionário/Aviso → **v0.3**.
- [x] **MyMemory — leitura formal dos termos comerciais (2026-08-10)**: revisado https://mymemory.translated.net/terms-and-conditions especificamente sobre uso comercial/distribuído. Achado: os termos não distinguem uso pessoal/comercial, não têm limite numérico explícito (mas proíbem "traduzir mais de um parágrafo por vez", que `traduzir_com_mymemory()` já cumpre por chamar por segmento único), não exigem atribuição, e restringem redistribuição de "Public Data" (já respeitado — só o Claude contribui pro dicionário público). Ponto de atenção levantado pra advogada avaliar: cláusula proíbe "revender os serviços da Translated sem consentimento expresso" — ambíguo pra um app gratuito que só embute chamadas passthrough à API, não cobra pelo uso do MyMemory especificamente. Interpretação final é julgamento jurídico, não técnico.
