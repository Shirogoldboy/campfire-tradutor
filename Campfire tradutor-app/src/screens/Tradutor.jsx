...
const UI = {
  'português brasileiro coloquial': {
    titulo: '🔥 Campfire Tradutor',
    selecionar: '+ Selecionar arquivo',
    selecionarPasta: '+ Selecionar pasta',
    traduzir: 'Traduzir →',
    traduzindo: '⏳ Traduzindo...',
    concluido: '✅ Concluído! Veja os arquivos gerados na pasta.',
    erro: '❌ Erro na tradução. Veja o log abaixo.',
    voltar: '← Voltar',
  },
  'português europeu': {
    titulo: '🔥 Campfire Tradutor',
    selecionar: '+ Selecionar ficheiro',
    selecionarPasta: '+ Selecionar pasta',
    traduzir: 'Traduzir →',
    traduzindo: '⏳ A traduzir...',
    concluido: '✅ Concluído! Veja os ficheiros gerados na pasta.',
    erro: '❌ Erro na tradução. Veja o registo abaixo.',
    voltar: '← Voltar',
  },
  'english': {
    titulo: '🔥 Campfire Translator',
    selecionar: '+ Select file',
    selecionarPasta: '+ Select folder',
    traduzir: 'Translate →',
    traduzindo: '⏳ Translating...',
    concluido: '✅ Done! Check the generated files in the folder.',
    erro: '❌ Translation error. Check the log below.',
    voltar: '← Back',
  },
  'español': {
    titulo: '🔥 Campfire Traductor',
    selecionar: '+ Seleccionar archivo',
    selecionarPasta: '+ Seleccionar carpeta',
    traduzir: 'Traducir →',
    traduzindo: '⏳ Traduciendo...',
    concluido: '✅ ¡Listo! Revisa los archivos generados en la carpeta.',
    erro: '❌ Error en la traducción. Revisa el registro abajo.',
    voltar: '← Volver',
  },
  'français': {
    titulo: '🔥 Campfire Traducteur',
    selecionar: '+ Sélectionner un fichier',
    selecionarPasta: '+ Sélectionner un dossier',
    traduzir: 'Traduire →',
    traduzindo: '⏳ Traduction en cours...',
    concluido: '✅ Terminé ! Consultez les fichiers générés dans le dossier.',
    erro: '❌ Erreur de traduction. Voir le journal ci-dessous.',
    voltar: '← Retour',
  },
  'deutsch': {
    titulo: '🔥 Campfire Übersetzer',
    selecionar: '+ Datei auswählen',
    selecionarPasta: '+ Ordner auswählen',
    traduzir: 'Übersetzen →',
    traduzindo: '⏳ Übersetze...',
    concluido: '✅ Fertig! Sieh dir die erstellten Dateien im Ordner an.',
    erro: '❌ Übersetzungsfehler. Siehe Protokoll unten.',
    voltar: '← Zurück',
  },
  'italiano': {
    titulo: '🔥 Campfire Traduttore',
    selecionar: '+ Seleziona file',
    selecionarPasta: '+ Seleziona cartella',
    traduzir: 'Traduci →',
    traduzindo: '⏳ Traduzione in corso...',
    concluido: '✅ Fatto! Controlla i file generati nella cartella.',
    erro: '❌ Errore di traduzione. Vedi il registro sotto.',
    voltar: '← Indietro',
  },
  'japonês': {
    titulo: '🔥 Campfire 翻訳',
    selecionar: '+ ファイルを選択',
    selecionarPasta: '+ フォルダを選択',
    traduzir: '翻訳する →',
    traduzindo: '⏳ 翻訳中...',
    concluido: '✅ 完了！フォルダ内の生成ファイルを確認してください。',
    erro: '❌ 翻訳エラー。以下のログを確認してください。',
    voltar: '← 戻る',
  },
  'coreano': {
    titulo: '🔥 Campfire 번역기',
    selecionar: '+ 파일 선택',
    selecionarPasta: '+ 폴더 선택',
    traduzir: '번역하기 →',
    traduzindo: '⏳ 번역 중...',
    concluido: '✅ 완료! 폴더에서 생성된 파일을 확인하세요.',
    erro: '❌ 번역 오류. 아래 로그를 확인하세요.',
    voltar: '← 뒤로',
  },
  'chinês simplificado': {
    titulo: '🔥 Campfire 翻译器',
    selecionar: '+ 选择文件',
    selecionarPasta: '+ 选择文件夹',
    traduzir: '翻译 →',
    traduzindo: '⏳ 翻译中...',
    concluido: '✅ 完成！请查看文件夹中生成的文件。',
    erro: '❌ 翻译错误。请查看下方日志。',
    voltar: '← 返回',
  },
  'russo': {
    titulo: '🔥 Campfire Переводчик',
    selecionar: '+ Выбрать файл',
    selecionarPasta: '+ Выбрать папку',
    traduzir: 'Перевести →',
    traduzindo: '⏳ Перевод...',
    concluido: '✅ Готово! Проверьте созданные файлы в папке.',
    erro: '❌ Ошибка перевода. Смотрите журнал ниже.',
    voltar: '← Назад',
  },
  'árabe': {
    titulo: '🔥 Campfire مترجم',
    selecionar: '+ اختر ملفاً',
    selecionarPasta: '+ اختر مجلداً',
    traduzir: '→ ترجمة',
    traduzindo: '⏳ جارٍ الترجمة...',
    concluido: '✅ تم! تحقق من الملفات الناتجة في المجلد.',
    erro: '❌ خطأ في الترجمة. راجع السجل أدناه.',
    voltar: 'رجوع →',
  },
}
...
function parsearProgresso(logs) {
  ...
  for (let i = logs.length - 1; i >= 0; i--) {
    ...
    if (blocoMatch) {
      const bloco = parseInt(blocoMatch[1])
      const atual = parseInt(blocoMatch[2])
      const total = parseInt(blocoMatch[3])
      const totalBlocos = Math.ceil(total / 5)
      fase = `Varrendo bloco ${bloco}/${totalBlocos} — ${atual}MB / ${total}MB`
    ...
    if (batchMatch) {
      const atual = parseInt(batchMatch[1])
    ...
    if (linha.includes('Aplicando traduções')) {
      fase = 'Aplicando traduções...'
      pct = 92
      return { fase, pct }
    }
    ...
    if (parteMatch) {
      const atual = parseInt(parteMatch[1])
      const total = parseInt(parteMatch[2])
      fase = `Traduzindo parte ${atual} / ${total}`
      pct = Math.round((atual / total) * 90)
      return { fase, pct }
    }

    if (linha.includes('Copiando ISO')) {
      fase = 'Copiando ISO...'
      pct = 3
      return { fase, pct }
    }
...
export default function Tradutor({ modo, onVoltar }) {
  ...
  async function selecionarArquivo() {
    // Passa as extensões do modo pro seletor de arquivo
    const path = await window.api.selectFile(modo?.exts)
    if (path) { setArquivo(path); setTipo('arquivo'); setLogs([]); setStatus('idle') }
  }

  async function selecionarPasta() {
    const path = await window.api.selectFolder()
    if (path) { setArquivo(path); setTipo('pasta'); setLogs([]); setStatus('idle') }
  }
  ...
  return (
    <div style={{ ...s.container, direction: isRTL ? 'rtl' : 'ltr' }}>
      ...
      {/* Cabeçalho com modo ativo e botão voltar */}
      ...
      {/* Extensões aceitas pelo modo */}
      {modo?.desc && (
        <p style={s.extsAceitas}>Extensões aceitas: {modo.desc}</p>
      )}
      ...
      {status === 'erro' && <p style={s.erro}>{t.erro}</p>}
